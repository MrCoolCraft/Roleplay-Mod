...
public class mcreator_cityPlainsDimension {
	...
	static {
		portal = (BlockTutorialPortal) (new BlockTutorialPortal().setUnlocalizedName("cityPlainsDimension_portal"));
		block = (ModTrigger) (new ModTrigger().setUnlocalizedName("cityPlainsDimension_trigger"));
	}

	public mcreator_cityPlainsDimension() {
	}

	public static DimensionType dtype = DimensionType.register("cityPlainsDimension", "_cityPlainsDimension", DIMID,
			mcreator_cityPlainsDimension.WorldProviderMod.class, false);

	public void load(FMLInitializationEvent event) {

		portal.setRegistryName("cityplainsdimension_portal");
		GameRegistry.register(portal);
		GameRegistry.register(new ItemBlock(portal).setRegistryName(portal.getRegistryName()));

		block.setRegistryName("cityplainsdimension_trigger");
		GameRegistry.register(block);
		DimensionManager.registerDimension(DIMID, dtype);

		if (event.getSide() == Side.CLIENT)
			Minecraft.getMinecraft().getRenderItem().getItemModelMesher()
					.register(block, 0, new ModelResourceLocation("localminecrafttestmod:cityplainsdimension_trigger", "inventory"));

		GameRegistry.addRecipe(new ItemStack(block, 1), new Object[]{"XXX", "X45", "XXX", Character.valueOf('4'), new ItemStack(Items.DYE, 1, 10),
				Character.valueOf('5'), Items.STICK,});
	...
	public static class WorldProviderMod extends WorldProvider {
		...
		public IChunkGenerator createChunkGenerator() {
			return new ChunkProviderModded(this.world, this.world.getSeed() - 19636);
		}
					...
					for (int i1 = -2; i1 <= 2; ++i1) {
						for (int j1 = -1; j1 < 3; ++j1) {
							...
							this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2), flag
									? Blocks.PURPUR_DOUBLE_SLAB.getDefaultState()
	...
	public static class BlockTutorialPortal extends Block {
		...
		public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4) {
			byte b0 = 0;
			byte b1 = 0;
			if (getBlock(par1World, par2 - 1, par3, par4) == Blocks.PURPUR_DOUBLE_SLAB
					|| getBlock(par1World, par2 + 1, par3, par4) == Blocks.PURPUR_DOUBLE_SLAB) {
				b0 = 1;
			}
			if (getBlock(par1World, par2, par3, par4 - 1) == Blocks.PURPUR_DOUBLE_SLAB
					|| getBlock(par1World, par2, par3, par4 + 1) == Blocks.PURPUR_DOUBLE_SLAB) {
				b1 = 1;
			}
			if (b0 == b1) {
				return false;
				...
				for (l = -1; l <= 2; ++l) {
					for (i1 = -1; i1 <= 3; ++i1) {
						...
						if (l != -1 && l != 2 || i1 != -1 && i1 != 3) {
							...
							if (flag) {
								if (j1 != Blocks.PURPUR_DOUBLE_SLAB) {
									return false;
								}
							}
							/*
		...
		public void neighborChanged(IBlockState state, World par1World, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			...
			if (getBlock(par1World, par2, i1 - 1, par4) != Blocks.PURPUR_DOUBLE_SLAB) {
				...
				if (j1 == 3 && getBlock(par1World, par2, i1 + j1, par4) == Blocks.PURPUR_DOUBLE_SLAB) {
					...
					if (flag && flag1) {
						...
						if ((getBlock(par1World, par2 + b0, par3, par4 + b1) != Blocks.PURPUR_DOUBLE_SLAB || getBlock(par1World, par2 - b0, par3,
								par4 - b1) != this)
								&& (getBlock(par1World, par2 - b0, par3, par4 - b1) != Blocks.PURPUR_DOUBLE_SLAB || getBlock(par1World, par2 + b0,
										par3, par4 + b1) != this)) {
							par1World.setBlockToAir(new BlockPos(par2, par3, par4));
						}
					}
	...
	public static class ChunkProviderModded implements IChunkGenerator {
		protected static final IBlockState field_185982_a = Blocks.GRASS.getDefaultState();
		...
		private IBlockState oceanBlock = Blocks.DIRT.getDefaultState();
		...
		protected static final IBlockState GRAVEL = Blocks.GRASS.getDefaultState();
			...
			for (int j1 = 255; j1 >= 0; --j1) {
				if (j1 <= rand.nextInt(5)) {
					...
					if (iblockstate2.getMaterial() == Material.AIR) {
						j = -1;
					} else if (iblockstate2.getBlock() == Blocks.GRASS) {
						if (j == -1) {
							if (k <= 0) {
								iblockstate = AIR;
		...
		public void populate(int x, int z) {
			...
			if (Biome != Biomes.DESERT && Biome != Biomes.DESERT_HILLS && this.settings.useWaterLakes && !flag
				...
				if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, this.worldObj, this.rand, x, z, flag,
					...
					(new WorldGenLakes(Blocks.DIRT)).generate(this.worldObj, this.rand, blockpos.add(i1, j1, k1));
				}
			...
			if (!flag && this.rand.nextInt(this.settings.lavaLakeChance / 10) == 0 && this.settings.useLavaLakes)
				if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, this.worldObj, this.rand, x, z, flag,
					...
					if (l2 < this.worldObj.getSeaLevel() || this.rand.nextInt(this.settings.lavaLakeChance / 8) == 0) {
						(new WorldGenLakes(Blocks.DIRT)).generate(this.worldObj, this.rand, blockpos.add(i2, l2, k3));
					}
				}
