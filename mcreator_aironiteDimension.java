...
public class mcreator_aironiteDimension {
	...
	static {
		portal = (BlockTutorialPortal) (new BlockTutorialPortal().setUnlocalizedName("aironiteDimension_portal"));
		block = (ModTrigger) (new ModTrigger().setUnlocalizedName("aironiteDimension_trigger"));
	}

	public mcreator_aironiteDimension() {
	}

	public static DimensionType dtype = DimensionType.register("aironiteDimension", "_aironiteDimension", DIMID,
			mcreator_aironiteDimension.WorldProviderMod.class, false);

	public void load(FMLInitializationEvent event) {

		portal.setRegistryName("aironitedimension_portal");
		GameRegistry.register(portal);
		GameRegistry.register(new ItemBlock(portal).setRegistryName(portal.getRegistryName()));

		block.setRegistryName("aironitedimension_trigger");
		GameRegistry.register(block);
		DimensionManager.registerDimension(DIMID, dtype);

		if (event.getSide() == Side.CLIENT)
			Minecraft.getMinecraft().getRenderItem().getItemModelMesher()
					.register(block, 0, new ModelResourceLocation("roleplaymod:aironitedimension_trigger", "inventory"));

		GameRegistry.addRecipe(new ItemStack(block, 1), new Object[]{"XXX", "X45", "XXX", Character.valueOf('4'), new ItemStack(Blocks.WOOL, 1, 0),
				Character.valueOf('5'), new ItemStack(Blocks.WOOL, 1, 5),});
	...
	public static class WorldProviderMod extends WorldProvider {
		...
		public Vec3d getFogColor(float par1, float par2) {
			return new Vec3d(0.2D, 1.0D, 0.8D);
		}

		@Override
		public IChunkGenerator createChunkGenerator() {
			return new ChunkProviderModded(this.world, this.world.getSeed() - 8234);
		}
					...
					for (int i1 = -2; i1 <= 2; ++i1) {
						for (int j1 = -1; j1 < 3; ++j1) {
							...
							this.worldServerInstance.setBlockState(new BlockPos(k1, l1, i2),
									flag ? Blocks.PLANKS.getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			...
			for (j3 = 0; j3 < 4; ++j3) {
				for (k3 = 0; k3 < 4; ++k3) {
					for (l3 = -1; l3 < 4; ++l3) {
						...
						this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4), flag1 ? Blocks.PLANKS.getDefaultState() : iblockstate, 2);
					}
				}
	...
	public static class BlockTutorialPortal extends Block {
		...
		public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4) {
			byte b0 = 0;
			byte b1 = 0;
			if (getBlock(par1World, par2 - 1, par3, par4) == Blocks.PLANKS || getBlock(par1World, par2 + 1, par3, par4) == Blocks.PLANKS) {
				b0 = 1;
			}
			if (getBlock(par1World, par2, par3, par4 - 1) == Blocks.PLANKS || getBlock(par1World, par2, par3, par4 + 1) == Blocks.PLANKS) {
				b1 = 1;
			}
			if (b0 == b1) {
				return false;
		...
		public void neighborChanged(IBlockState state, World par1World, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			...
			if (getBlock(par1World, par2, i1 - 1, par4) != Blocks.PLANKS) {
				...
				if (j1 == 3 && getBlock(par1World, par2, i1 + j1, par4) == Blocks.PLANKS) {
					...
					if (flag && flag1) {
						...
						if ((getBlock(par1World, par2 + b0, par3, par4 + b1) != Blocks.PLANKS || getBlock(par1World, par2 - b0, par3, par4 - b1) != this)
								&& (getBlock(par1World, par2 - b0, par3, par4 - b1) != Blocks.PLANKS || getBlock(par1World, par2 + b0, par3, par4
										+ b1) != this)) {
							par1World.setBlockToAir(new BlockPos(par2, par3, par4));
						}
					}
	...
	public static class ChunkProviderModded implements IChunkGenerator {
		protected static final IBlockState field_185982_a = mcreator_aironiteOre.block.getDefaultState();
		...
		private IBlockState oceanBlock = Blocks.FLOWING_WATER.getDefaultState();
		...
		public ChunkProviderModded(World worldIn, long seed) {
			...
			this.settings = ChunkProviderSettings.Factory.jsonToFactory("").build();
		...
		public void replaceBiomeBlocks(int x, int z, ChunkPrimer primer, Biome[] biomesIn) {
			...
			for (int i = 0; i < 16; ++i) {
				for (int j = 0; j < 16; ++j) {
					...
					generateBiomeTerrain(this.worldObj, this.rand, primer, x * 16 + i, z * 16 + j, this.field_186002_u[j + i * 16], Biome);
				}
			}
		}
		...
		protected static final IBlockState GRAVEL = mcreator_aironiteOre.block.getDefaultState();

		public void generateBiomeTerrain(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int x, int z, double noiseVal, Biome Biome) {
			...
			for (int j1 = 255; j1 >= 0; --j1) {
				if (j1 <= rand.nextInt(5)) {
					...
					if (iblockstate2.getMaterial() == Material.AIR) {
						j = -1;
					} else if (iblockstate2.getBlock() == mcreator_aironiteOre.block) {
						if (j == -1) {
							if (k <= 0) {
								iblockstate = AIR;
								iblockstate1 = mcreator_aironiteOre.block.getDefaultState();
							...
							if (j1 < i && (iblockstate == null || iblockstate.getMaterial() == Material.AIR)) {
								iblockstate1 = Blocks.FLOWING_WATER.getDefaultState();
							}
		...
		public void populate(int x, int z) {
			...
			if (Biome != Biomes.DESERT && Biome != Biomes.DESERT_HILLS && this.settings.useWaterLakes && !flag
				...
				if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, this.worldObj, this.rand, x, z, flag,
					...
					(new WorldGenLakes(Blocks.FLOWING_WATER)).generate(this.worldObj, this.rand, blockpos.add(i1, j1, k1));
				}
			...
			if (!flag && this.rand.nextInt(this.settings.lavaLakeChance / 10) == 0 && this.settings.useLavaLakes)
				if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, this.worldObj, this.rand, x, z, flag,
					...
					if (l2 < this.worldObj.getSeaLevel() || this.rand.nextInt(this.settings.lavaLakeChance / 8) == 0) {
						(new WorldGenLakes(Blocks.FLOWING_WATER)).generate(this.worldObj, this.rand, blockpos.add(i2, l2, k3));
					}
				}
