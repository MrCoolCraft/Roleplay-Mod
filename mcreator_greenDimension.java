...
public class mcreator_greenDimension {
	...
	static {
		portal = (BlockTutorialPortal) (new BlockTutorialPortal().setUnlocalizedName("greenDimension_portal"));
		block = (ModTrigger) (new ModTrigger().setUnlocalizedName("greenDimension_trigger"));
	}

	public mcreator_greenDimension() {
	}

	public static DimensionType dtype = DimensionType.register("greenDimension", "_greenDimension", DIMID,
			mcreator_greenDimension.WorldProviderMod.class, false);

	public void load(FMLInitializationEvent event) {

		portal.setRegistryName("greendimension_portal");
		GameRegistry.register(portal);
		GameRegistry.register(new ItemBlock(portal).setRegistryName(portal.getRegistryName()));

		block.setRegistryName("greendimension_trigger");
		GameRegistry.register(block);
		DimensionManager.registerDimension(DIMID, dtype);

		if (event.getSide() == Side.CLIENT)
			Minecraft.getMinecraft().getRenderItem().getItemModelMesher()
					.register(block, 0, new ModelResourceLocation("roleplaymod:greendimension_trigger", "inventory"));
	...
	public static class WorldProviderMod extends WorldProvider {
		...
		public IChunkGenerator createChunkGenerator() {
			return new ChunkProviderModded(this.world, this.world.getSeed() - 19536);
		}
				...
				for (k3 = 0; k3 < 4; ++k3) {
					for (l3 = -1; l3 < 4; ++l3) {
						...
						this.worldServerInstance.setBlockState(new BlockPos(i4, j4, k4),
								flag1 ? Blocks.HARDENED_CLAY.getDefaultState() : iblockstate, 2);
					}
				}
	...
	public static class BlockTutorialPortal extends Block {
		...
		public boolean tryToCreatePortal(World par1World, int par2, int par3, int par4) {
			byte b0 = 0;
			byte b1 = 0;
			if (getBlock(par1World, par2 - 1, par3, par4) == Blocks.HARDENED_CLAY
					|| getBlock(par1World, par2 + 1, par3, par4) == Blocks.HARDENED_CLAY) {
				b0 = 1;
			}
			if (getBlock(par1World, par2, par3, par4 - 1) == Blocks.HARDENED_CLAY
					|| getBlock(par1World, par2, par3, par4 + 1) == Blocks.HARDENED_CLAY) {
				b1 = 1;
			}
			if (b0 == b1) {
				return false;
		...
		public void neighborChanged(IBlockState state, World par1World, BlockPos pos, Block neighborBlock, BlockPos fromPos) {
			...
			if (getBlock(par1World, par2, i1 - 1, par4) != Blocks.HARDENED_CLAY) {
				...
				if (j1 == 3 && getBlock(par1World, par2, i1 + j1, par4) == Blocks.HARDENED_CLAY) {
					...
					if (flag && flag1) {
						...
						if ((getBlock(par1World, par2 + b0, par3, par4 + b1) != Blocks.HARDENED_CLAY || getBlock(par1World, par2 - b0, par3, par4
								- b1) != this)
								&& (getBlock(par1World, par2 - b0, par3, par4 - b1) != Blocks.HARDENED_CLAY || getBlock(par1World, par2 + b0, par3,
										par4 + b1) != this)) {
							par1World.setBlockToAir(new BlockPos(par2, par3, par4));
						}
					}
			...
			if (par5Random.nextInt(100) == 0) {
				par1World.playSound((double) pos.getX() + 0.5D, (double) pos.getY() + 0.5D, (double) pos.getZ() + 0.5D,
						(net.minecraft.util.SoundEvent) net.minecraft.util.SoundEvent.REGISTRY.getObject(new ResourceLocation(
								("CUSTOM:mod.music.jellycastle.mdk"))), SoundCategory.BLOCKS, 0.5F, par5Random.nextFloat() * 0.4F + 0.8F, false);
			}
			for (int l = 0; l < 4; ++l) {
	...
	public static class ChunkProviderModded implements IChunkGenerator {
		protected static final IBlockState field_185982_a = mcreator_aironiteBlock.block.getDefaultState();
		...
		protected static final IBlockState GRAVEL = mcreator_aironiteBlock.block.getDefaultState();
			...
			for (int j1 = 255; j1 >= 0; --j1) {
				if (j1 <= rand.nextInt(5)) {
					...
					if (iblockstate2.getMaterial() == Material.AIR) {
						j = -1;
					} else if (iblockstate2.getBlock() == mcreator_aironiteBlock.block) {
						if (j == -1) {
							if (k <= 0) {
								iblockstate = AIR;
								iblockstate1 = mcreator_aironiteBlock.block.getDefaultState();
		...
		public void populate(int x, int z) {
			...
			if (Biome != Biomes.DESERT && Biome != Biomes.DESERT_HILLS && this.settings.useWaterLakes && !flag
				...
				if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, this.worldObj, this.rand, x, z, flag,
					...
					(new WorldGenLakes(Blocks.AIR)).generate(this.worldObj, this.rand, blockpos.add(i1, j1, k1));
				}
			...
			if (!flag && this.rand.nextInt(this.settings.lavaLakeChance / 10) == 0 && this.settings.useLavaLakes)
				if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, this.worldObj, this.rand, x, z, flag,
					...
					if (l2 < this.worldObj.getSeaLevel() || this.rand.nextInt(this.settings.lavaLakeChance / 8) == 0) {
						(new WorldGenLakes(Blocks.AIR)).generate(this.worldObj, this.rand, blockpos.add(i2, l2, k3));
					}
				}
